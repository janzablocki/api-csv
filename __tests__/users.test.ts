...
describe('Get user by id from database', () => {
  test('Get user by id from database - successfully gets user', async () => {
    let insertResponse = await pool.query('INSERT INTO users (first_name, last_name, email, role_description, team) VALUES ($1, $2, $3, $4, $5) RETURNING id', ['Jane', 'Doe', 'janedoe@example.com', 'Test', 'Test']);
